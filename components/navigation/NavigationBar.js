...
function NavigationBar() {
  ...
  return (
    <Flex justify="space-between" px={["4", "4", "4", "28", "40"]} py={["4"]} backdropFilter="blur(8px)" alignItems={["start", "start", "start", "center", "center"]} bg={["rgba(0, 0, 0, .35)"]} border="1px" borderColor="rgb(69, 51, 33)" position="fixed" zIndex={99} w="full" h={responsiveDisplay === "flex" ? ["100vh", "100vh", "100vh", "min-content", "min-content"] : "min-content"} direction={["column", "column", "column", "row", "row"]}>
        ...
        <Flex alignItems={["start", "start", "start", "center", "center"]} display={[responsiveDisplay, responsiveDisplay, responsiveDisplay, "flex", "flex"]} direction={["column", "column", "column", "row", "row"]}>
            {navLinks.map(({name, path}) => (
                <Link key={name} href={path} passHref>
                    <Box ml={["0", "0", "0", "8", "8"]} fontFamily="'Merienda One', cursive" cursor="pointer" fontWeight={["normal", "normal", "normal", "bold", "bold"]} fontSize={["4xl", "4xl", "4xl", "lg", "lg"]} color="white" _hover={{ color: ["rgba(255, 204, 153, 1)"] }}>{name}</Box>
                </Link>
            ))}
        </Flex>
    </Flex>
